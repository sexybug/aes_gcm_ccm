...
int main()
{
    ...
    ccm_updateAData(&ctx, std_A, 7, 0);
    ccm_updateAData(&ctx, std_A + 7, A_len - 7, 1);

    int out_len1, out_len2, out_len3;
    ccm_update(&ctx, std_P, 7, enc_out, &out_len1);
    ccm_update(&ctx, std_P + 7, P_len - 7, enc_out + out_len1, &out_len2);
    ccm_final(&ctx, enc_out + out_len1 + out_len2, &out_len3, enc_Tag);
