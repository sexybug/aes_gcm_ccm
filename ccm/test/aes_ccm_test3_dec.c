...
int main()
{
    ...
    CCM_CTX ctx;
    int ret = ccm_init(&ctx, cipher, CCM_DECRYPT, std_K, K_len, std_N, N_len, A_len, P_len, T_len);
    if (ret != 1)
    {
        return -1;
    }
    ...
    ccm_update(&ctx, std_C, P_len, dec_out, &out_len);
    ccm_final(&ctx, dec_out + out_len, &out_len, tag);
